...
def delay4():
    root.after(36000000, root.destroy) 
...
controls_frame.grid(row=2, column=0, padx =0,pady=0)


# FRAME -Volume Label
volume_frame = LabelFrame(master_frame, text="Volume",fg = "white", borderwidth=0, bg = "#374045")
volume_frame.config(bg = "#374045",borderwidth=10)
volume_frame.grid(row=2, column=1, padx=15,pady = 10, rowspan = 1)

# FRAME - playlist
song_box = Listbox(master_frame, bg="#e1d89f", fg="white", width=35, height = 15, selectbackground="#ffe227", selectforeground="black",borderwidth=10)
song_box.grid(row=0, column=1, pady = 6, padx = 7, columnspan = 20)
...
volume_meter.grid(row=2, column=10, padx=0)
...
back_button.grid(row=2, column=1, padx=2)
forward_button.grid(row=2, column=5, padx=2)
play_button.grid(row=2, column=3, padx=2)
pause_button.grid(row=2, column=2, padx=2)
stop_button.grid(row=2, column=4, padx=2)
...
submenu.add_command(label = "30 seconds", command = delay1)
submenu.add_command(label = "1 minute", command = delay2)
submenu.add_command(label = "5 minutes", command = delay3)
...
my_slider = ttk.Scale(master_frame, from_=0, to=100, orient=HORIZONTAL, value=0, command=slide, length=200)
my_slider.grid(row=1, column=0, pady=0)


#Volume Slider
volume_slider = ttk.Scale(volume_frame, from_=0, to=1, orient=HORIZONTAL, value=1, command=volume, length=125)
volume_slider.pack(pady=10,padx=30)
